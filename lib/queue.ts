...
export async function enqueue<T = unknown>(type: string, payload: T): Promise<Task<T>> {
  const task: Task<T> = {
...
async function markJobProcessing(id: string) {
  await prisma.job.update({ where: { id }, data: { status: 'processing', updatedAt: new Date() } });
}
...
async function claimNextJob(): Promise<{ id: string; type: string; payload: string; attempts: number; maxAttempts: number } | null> {
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        const nextRetryAt = attempts < task.maxAttempts ? new Date(Date.now() + 5000) : null;
