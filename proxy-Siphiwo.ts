...
function buildCsp({ nonce, isProd }: { nonce?: string; isProd: boolean }): string {
  ...
  // Dev additionally needs 'unsafe-eval': React requires it in development
  // mode for debugging features (callstack reconstruction); production never
  // uses eval, so it is omitted there.
